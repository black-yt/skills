...
| 03 | 列出 17 个独立需求及对应代码和 PDF；说明按场景分组生成、命名合集、文字/字体/位图检查、渲染预览和失败处理。 | render_examples.py, verify_pdf.py, diagrams, statistics, extension, leaderboard, training, ToUnicode, previews, outputs, requirements.txt | 首次运行示例前必须读取；选择最接近需求或批量生成场景前必须读取；交付 PDF、检查复制能力或排查输出质量前必须读取 | [references/03_examples_and_validation.md](references/03_examples_and_validation.md) |
...
- **预览**：`outputs/previews/` 中的 PNG 从成品 PDF 渲染，仅供目视检查；正式交付仍使用 PDF。
- **生成物**：`outputs/`、`.venv/` 与 Python 缓存已由本 skill 的 `.gitignore` 排除。分发代码后运行示例即可重新生成。
