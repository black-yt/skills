...
- 在本仓库的 `SKILL.md`、`references/*.md`、`README.md` 或网页预览内容中写 GitHub Markdown 公式时，必须参考 `ai-conference-paper-writing/references/markdown-writing-templates.md` 的 `GitHub Markdown 公式` 经验；优先使用 `math` 围栏，避免表格内复杂公式、`x_{<t}`、高风险宏和未转义 HTML 标签。
- 未知值使用占位符，不要编造。
...
- 同一经验写入多个 skill 或主文件/reference 时，使用同一份覆盖清单做一致性检查；差异只能来自使用场景不同，不能让一个位置缺少关键示例、验证步骤或失败处理。
...
- 需要写具体文档链接时，先说明如何获取当前版本，例如 `python -c 'import vllm; print(vllm.__version__)'`，再说明如何选择对应版本文档。
...
- 同一个工作流如果写入多个 skill，例如集群内网服务 SSH 转发和 LLM 部署本地访问，核心步骤、参数含义、验证命令和排错顺序必须保持一致；只允许替换具体登录地址、路径或场景说明。
