...
| --- | --- | --- | --- | --- |
| 1 | 概括使用开发机、CPU/GPU worker 和 rjob 时必须遵守的安全边界、环境禁改规则和当前测试状态。 | 覆盖范围、核心原则、环境安全、实测状态、提交前检查、排错 | 默认读取 | `SKILL.md` |
| 2 | 说明如何用交互式 SSH 进入开发机、在哪些路径放代码/数据，以及如何用 `git apply`、`perl`、`sed`、`scp` 安全编辑远端文件。 | 登录、路径、交互式 SSH、远端编辑、`git apply`、`perl`、`sed`、`scp` | 需要登录开发机、编辑远端文件、同步本地改动或传输文件时必须读取 | [references/remote-access-and-editing.md](references/remote-access-and-editing.md) |
| 3 | 记录代理启停、CPU/GPU 节点网络差异、共享存储目录、模型权重位置和分区可用性版本说明。 | 网络代理、模型权重、公共软件路径、存储、分区状态 | 需要联网、访问私网服务、查模型权重、放置大文件或选择分区时必须读取 | [references/network-storage-resources.md](references/network-storage-resources.md) |
| 4 | 给出 `rlaunch` 申请交互式 CPU/GPU worker 的原始命令、资源配比、分区差异和启动后的检查方式。 | `rlaunch`、CPU worker、GPU worker、资源检查 | 需要启动交互式 CPU/GPU worker 或做资源预测时必须读取 | [references/rlaunch-workers.md](references/rlaunch-workers.md) |
| 5 | 提供正式 `rjob` CPU/GPU 作业的提交、查询、删除、脚本初始化、CUDA/conda 恢复和 host-network 注意事项。 | `rjob`、CPU task、GPU job、作业脚本骨架 | 需要提交、查询、删除或编写正式 rjob 任务时必须读取 | [references/rjob-tasks.md](references/rjob-tasks.md) |
| 6 | 说明部署服务后如何从日志获取内网 IP、设置 no_proxy、做多节点协作，并用 SSH local port forwarding 转发到本地访问。 | 服务部署、SSH local port forwarding、复杂协作模式 | 需要部署模型/API 服务、获取服务 IP、从本地访问集群内网服务或多节点协作时必须读取 | [references/service-deployment-and-collaboration.md](references/service-deployment-and-collaboration.md) |
