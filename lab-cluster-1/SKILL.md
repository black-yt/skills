---
name: lab-cluster-1
description: "当需要在 lab cluster 1 / PJLAB 开发机上交互式登录后，通过已实测的原始 rlaunch 或 rjob 命令使用 CPU/GPU worker、提交训练或部署任务、检查资源申请、处理 ai4sdata/scieval 分区、挂载共享存储、设置 CPU worker 网络代理，或从开发机访问 worker 服务时使用；不要依赖 .bashrc 中的 gpu/cpu/pred 快捷函数。"
---
...
CPU rjob 可以运行，但本次外网代理测试没有通过：`curl https://www.google.com` 返回 `407 Proxy Authentication Required`。需要 CPU rjob 联网时，先和用户确认代理认证方案，再重新测试。
...
CPU 脚本可以设置代理，但本次 CPU rjob 外网代理返回 407；在 rjob 中使用外网前必须重新验证认证方案。CPU worker 的同类代理命令已实测成功。
...
访问私网服务前，如果项目有 `structai.add_no_proxy_if_private`，先调用：
